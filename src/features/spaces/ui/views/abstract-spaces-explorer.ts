...
export class AbstractSpacesExplorerView extends ItemView implements ViewportDelegate {
    ...
    public async focusFile(path: string) {
        ...
        let matchingNode = this.currentItems.find(item => item.id === path);
        ...
        if (uri) {
            ...
            matchingNode = this.currentItems.find(item => item.uri === uri);
            if (matchingNode) {
    ...
    private syncWithActiveFile() {
        if (this.plugin.settings.autoScrollToActiveFile && this.selectedSpace) {
