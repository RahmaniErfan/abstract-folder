...
Reverting the "Topic Note" auto-generation and implicit parenting logic. Shifting to a "Master Library Group" model where each library has exactly one Group in the UI, pointing to its physical root.
...
graph-engine.ts
Remove 
getImplicitTopicParent
.
Remove any calls to it within 
getFileRelationships
.
Ensure orphans remain orphans unless explicitly parented via YAML.
[MODIFY] 
shallow-sync-executor.ts
Remove 
cleanupAutoGeneratedRoots
.
Remove 
validateAndAutoGenerateRoots
.
Update 
execute()
 loop to remove calls to these methods.
Update 
ShallowSyncExecutorConfig
 to remove 
registerGroup
 (it will be handled by SyncManager instead).
[MODIFY] 
sanitization.ts
Remove 
getTopicRootNotePath
.
Keep 
sanitizeTopicName
 for Git command safety.
Master Library Group (New Logic)
[MODIFY] 
sync-manager.ts
Update 
startPublicSyncEngine
 to register exactly one Master Group per library.
Group Name: [Library] <LibraryName>
Target Path: Physical root of the library (e.g., Abstract Library/Wiki).
Note Creation Routing
[MODIFY] 
abstract-folder-toolbar.ts
Update "Create Note" logic: if a Library Master Group is active, route new notes to the library root folder.
Verification Plan
Manual Verification
Install/Sync a library.
Verify exactly one group [Library] Name is created.
Verify library folder structure is mirrored exactly from Git (natural orphans).
Verify "Create Note" while in the library group creates a file at the library root.
